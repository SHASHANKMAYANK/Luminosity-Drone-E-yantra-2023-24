...
class swift():
    ...
    def __init__(self):
        ...
        self.setpoint = [-6, -7,
                         20]  # whycon marker at the position of the dummy given in the scene. Make the whycon marker associated with position_to_hold dummy renderable and make changes accordingly5
    ...
    def perfect_landing(self):
        if (self.setpoint[2] > 34):
            ...
            self.q = self.drone_position[0]
            self.r = self.drone_position[1]
            self.s = self.drone_position[2]



            self.z = self.z + 1
            self.alien_location.pop(0)
            self.x = self.alien_type[0]
            self.alien_type.pop(0)
            if (len(self.alien_location) >= 1 and self.z >=0):

                    self.cdd.organism_type = self.x
                    self.cdd.whycon_x = self.q
                    self.cdd.whycon_y = self.r
                    self.cdd.whycon_z = self.s
    ...
    def pid(self):
        ...
        self.cmd.rcRoll = int(1500 - (
                    self.Kp[0] * self.roll_error + self.Kd[0] * (self.roll_error - self.roll_prev_error) + self.Ki[
                0] * self.roll_sum_error))
        self.cmd.rcPitch = int(1500 + (
                    self.Kp[1] * self.pitch_error + self.Kd[1] * (self.pitch_error - self.pitch_prev_error) + self.Ki[
                1] * self.pitch_sum_error))
        self.cmd.rcThrottle = int(1583 + (
                    self.throttle_error * self.Kp[2] + (self.throttle_error - self.throttle_prev_error) * self.Kd[
                2] + self.throttle_sum_error * self.Ki[2]))
        ...
        if (self.setpoint[0] == 11 and self.setpoint[1] == 11 and self.setpoint[2] == 37
            ...
            rospy.loginfo(self.drone_position[2])
                ...
                (self.setpoint[0] -0.2) <= self.drone_position[0] <= (self.setpoint[0] + 0.20)) and (
                      (self.setpoint[1] - 0.2) <= self.drone_position[1] <= (self.setpoint[1] + 0.2)) and (
                      (self.setpoint[2] - 0.2) <= self.drone_position[2] <= (self.setpoint[2] + 0.2))):
            self.perfect_landing()

        elif (self.setpoint[2] > 20 and (
                (self.setpoint[0] - 0.19) <= self.drone_position[0] <= (self.setpoint[0] + 0.19)) and (
                      (self.setpoint[1] - 0.19) <= self.drone_position[1] <= (self.setpoint[1] + 0.19)) and (
                      (self.setpoint[2] - 0.19) <= self.drone_position[2] <= (self.setpoint[2] + 0.19))):
            self.perfect_landing()

        elif (len(self.wp_setpoints) > self.prev_length and (
                (self.setpoint[0] - 0.19) <= self.drone_position[0] <= (self.setpoint[0] + 0.19)) and (
                      (self.setpoint[1] - 0.19) <= self.drone_position[1] <= (self.setpoint[1] + 0.19)) and (
                      (self.setpoint[2] - 0.19) <= self.drone_position[2] <= (self.setpoint[2] + 0.19))):
            rospy.loginfo(self.setpoint[0])
            rospy.loginfo(self.setpoint[1])
            rospy.loginfo(self.setpoint[2])
            if (self.setpoint[0] == 0 and self.setpoint[1] == 0 and self.setpoint[2] == 19 and ((self.setpoint[0] - 0.19) <= self.drone_position[0] <= (self.setpoint[0] + 0.19)) and (
                        (self.setpoint[1] - 0.19) <= self.drone_position[1] <= (self.setpoint[1] + 0.19)) and (
                        (self.setpoint[2] - 0.19) <= self.drone_position[2] <= (self.setpoint[2] + 0.19))):
            ...
            for self.i in range(0, len(self.wp_setpoints), 1):
                if ((self.setpoint[0] == self.wp_setpoints[self.i][0] and self.setpoint[1] == self.wp_setpoints[self.i][
                    1] and
                ...
                elif ((self.setpoint[0] == self.wp_setpoints[self.i][0] and self.setpoint[1] ==
                       self.wp_setpoints[self.i][1] and
                       self.setpoint[2] == self.wp_setpoints[self.i][2]) and (
                              (self.setpoint[0] - 0.19) <= self.drone_position[0] <= (self.setpoint[0] + 0.19)) and (
                              (self.setpoint[1] - 0.19) <= self.drone_position[1] <= (self.setpoint[1] + 0.19)) and (
                              (self.setpoint[2] - 0.19) <= self.drone_position[2] <= (self.setpoint[2] + 0.19)) and len(
...
if __name__ == '__main__':
    ...
    swift_drone = swift()
    r = rospy.Rate(
        20)  # specify rate in Hz based upon your desired PID sampling time, i.e. if desired sample time is 33ms specify rate as 30Hz

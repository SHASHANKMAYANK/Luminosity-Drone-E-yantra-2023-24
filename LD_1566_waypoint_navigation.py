...
class swift():
	...
	#	8. Add error_sum
		...
		for i in range(0,len(self.wp_setpoints)-1,1):
	
			if((self.setpoint[0]==self.wp_setpoints[i][0] and self.setpoint[1]==self.wp_setpoints[i][1] and self.setpoint[2]==self.wp_setpoints[i][2]) and ((self.setpoint[0]-0.19)<=self.drone_position[0]<=(self.setpoint[0]+0.19)) and((self.setpoint[1]-0.19)<=self.drone_position[1]<=(self.setpoint[1]+0.19)) and 		    				((self.setpoint[2]-0.19)<=self.drone_position[2]<=(self.setpoint[2]+0.19))) :
				
					self.setpoint[0]=self.wp_setpoints[i+1][0]
					self.setpoint[1]=self.wp_setpoints[i+1][1]
					self.setpoint[2]=self.wp_setpoints[i+1][2]
					print(i)
